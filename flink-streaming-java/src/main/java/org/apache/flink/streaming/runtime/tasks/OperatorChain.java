...
public abstract class OperatorChain<OUT, OP extends StreamOperator<OUT>>
    ...
    protected void sentAcknowledgeCheckpointEvent(long checkpointId) {
        if (operatorEventDispatcher == null) {
            return;
        }

        operatorEventDispatcher.getRegisteredOperators().forEach(
                x -> operatorEventDispatcher.getOperatorEventGateway(x).sendEventToCoordinator(
