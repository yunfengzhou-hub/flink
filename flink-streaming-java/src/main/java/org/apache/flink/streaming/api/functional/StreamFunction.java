...
class StreamFunction<T, R> implements Function<List<T>, List<R>> {
    ...
    List<StreamRecord> output;

    StreamFunction(DataStream<R> dataStream) throws Exception {
        StreamGraph graph = getStreamGraph(dataStream);
        validateDataStream(graph);
    ...
    private static void validateDataStream(StreamGraph graph) throws Exception {
        List<StreamNode> nodes = new ArrayList<>(graph.getStreamNodes());
    ...
    public List<R> apply(List<T> ts) {
        ...
        for(StreamRecord<R> record:this.output){
