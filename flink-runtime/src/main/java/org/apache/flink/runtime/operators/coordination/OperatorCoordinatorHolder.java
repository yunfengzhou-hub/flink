...
public class OperatorCoordinatorHolder
    ...
    /**
     * A map that manages a completable future for each subtask. Each future in this map has the
     * following semantics:
     *
     * <ul>
     *   <li>A future is created when the holder sends a {@link CloseGatewayEvent} with {@link
     *       #currentPendingCheckpointId} to a subtask.
     *   <li>The future is completed when the corresponding subtask returned an {@link
     *       AcknowledgeCheckpointEvent} with the {@link #currentPendingCheckpointId}, or if the
     *       subtask is not running.
     *   <li>The future is completed exceptionally if the subtask throws an exception on the
     *       received {@link CloseGatewayEvent} while it is still running.
     * </ul>
     *
    ...
    private OperatorCoordinatorHolder(
        ...
        this.currentPendingCheckpointId = OperatorCoordinator.NO_CHECKPOINT;
    ...
    public void executionAttemptFailed(int subtask, int attemptNumber, @Nullable Throwable reason) {
        ...
        if (!context.isConcurrentExecutionAttemptsSupported()) {
            abortAcknowledgeCloseGatewayFuture(
        ...
        mainThreadExecutor.execute(
                () -> {
                    ...
                    if (checkpointId == latestAttemptedCheckpointId) {
                        ...
                        currentPendingCheckpointId = OperatorCoordinator.NO_CHECKPOINT;
                    }
    ...
    private void abortAcknowledgeCloseGatewayFutures(String message) {
        acknowledgeCloseGatewayFutureMap
                .values()
                .forEach(x -> x.completeExceptionally(new FlinkException(message)));
        acknowledgeCloseGatewayFutureMap.clear();
    }

    private void abortAcknowledgeCloseGatewayFuture(
    ...
    public void abortCurrentTriggering() {
        ...
        mainThreadExecutor.execute(
                () -> {
                    abortAcknowledgeCloseGatewayFutures("Current triggering has been aborted");
                    currentPendingCheckpointId = OperatorCoordinator.NO_CHECKPOINT;
                    subtaskGatewayMap
                            .values()
